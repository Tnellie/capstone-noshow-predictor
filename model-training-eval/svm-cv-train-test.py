...
from prismSparkML import SupportVectorMachine as svm
...
def compute(
    ...
    """
    Perform hyperparameter tuning and validation for Support Vector Machine employee no-show prediction.
    
    This transform executes a comprehensive SVM workflow for binary classification:
    ...
    Support Vector Machines use kernel methods to find optimal decision boundaries in high-dimensional
    feature space, handling extreme class imbalance (0.15% no-show rate) through weighted samples and
    evaluated using precision-recall metrics appropriate for imbalanced classification.
    ...
    Outputs:
        cv_results: Cross-validation results containing performance metrics for all hyperparameter
            ...
            for each regularization configuration.
    ...
    # SVM cross-validation hyperparameter search
    cv_result_df = svm.CV_SupportVectorMachine(
        cv_df=cv_df,
        ...
        date_col="date",
        n_param_combos=4,
        param_overrides={"regParam": [0.01, 0.5, 0.1]},
    )
    ...
    trained_model = svm.train_svm_model(
    ...
    # # Train final model
    # final_svm_model = svm.train_svm_model(
